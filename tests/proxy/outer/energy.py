...
class EnergyProxy:
    ...
    def brush_instance_zones(self, instance_type):
        ...
        # if self.op.exists(outer_pos.the_shadow_keep_pos):
        ...
        if self.op.exists(outer_pos.into_battle_pos):
            self.op.close_page()

        end_energy = self.op.get_current_energy_value()

        logstep('副本闯关完成 ...')
        self.back_outer_view()
        return start_energy - end_energy

    @logwrap('回到 飞空艇外围')
    def back_outer_view(self):
        if self.op.exists(outer_pos.close_page_pos):
            self.op.close_page()
        if self.op.exists(outer_pos.close_page_pos):
            self.op.close_page()
    ...
    def verify_energy_consumption_of_lost_sector(self):
        ...
        """
        self.op.lost_sector()
        consume_energy = self.brush_instance_zones(outer_pos.bihna_pos)
    ...
    def verify_energy_consumption_of_the_shadow_keep(self):
        ...
        consume_energy = self.brush_instance_zones(outer_pos.mage_pos)
        assertion.assert_less_equal(consume_energy, 10)
    ...
    def verify_energy_consumption_of_potion(self):
        ...
        consume_energy = self.brush_instance_zones(outer_pos.potion_pos)
        assertion.assert_less_equal(consume_energy, 10)
    ...
    def verify_energy_consumption_of_the_gold(self):
        ...
        # 黄金谷如果是首次通关，不扣钥匙
        if int(current_level) < int(outer_pos.the_gold_level_pos):
            gold_key = 0
        else:
            gold_key = 1
    ...
    def verify_energy_consumption_of_elemental_valley(self):
        ...
        consume_energy = self.brush_instance_zones(outer_pos.frost_lord_pos)
...
if __name__ == '__main__':
    ...
    # ep.verify_energy_consumption_of_elemental_valley()
